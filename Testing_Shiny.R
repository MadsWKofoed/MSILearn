...
# NOTE: ref_mz_path is optional; if not found, we skip alignment (robust default).
process_msi_files <- function(imzml_path, ibd_path, ref_mz_path = NULL) {
  # Why: keep original basenames so .imzML can find its .ibd
  temp_dir <- tempdir()
  imzml_dst <- file.path(temp_dir, basename(imzml_path))
  ibd_dst   <- file.path(temp_dir, basename(ibd_path))
  file.copy(imzml_path, imzml_dst, overwrite = TRUE)
  file.copy(ibd_path,   ibd_dst,   overwrite = TRUE)
  
  # FIX: removed stray empty argument
  msi_data <- readImzML(
    imzml_dst,
  ...
  control_MSI_ref <- tryCatch({
    if (!is.null(ref_mz_path) && isTRUE(file.exists(ref_mz_path))) {
      ...
      # Fallback: no external ref; just use picked peaks
      control_mean %>%
        peakPick(SNR = 3) %>%
        process()
    }
  }, error = function(e) {
    # Last-resort fallback to proceed
