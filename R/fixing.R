...
# control_mean <- summarizeFeatures(msi_data, "mean")
# 
# writeMSIData(control_mean, "control_mean.imzML")
# 
# control_mean_loaded <- readMSIData("control_mean.imzML")
# 
# 
# 
# control_SNR_ref <- control_mean_loaded %>%
#   peakPick(SNR = 4)
# 
# 
# 
...
#   process()
# 
# writeMSIData(control_MSI_ref, "control_MSI_ref.imzML")
# 
# control_MSI_ref_loaded <- readMSIData("control_MSI_ref.imzML")
# 
# msi_data_binned <- bin(
#   msi_data,
#   ref = mz(control_MSI_ref_loaded),
#   tolerance = 0.5,
#   units = "mz",
#   BPPARAM = BiocParallel::bpparam()
# ) %>% process()
...
# 
# 
# save_msi_to_gridfs <- function(msi_obj, base_name, mongo_url = "mongodb://localhost", db = "cardinal_data", prefix = "fs") {
#   # 1) Skriv imzML + ibd til en midlertidig mappe
#   tmp <- tempfile("imzml_"); dir.create(tmp)
#   imzml_path <- file.path(tmp, paste0(base_name, ".imzML"))
#   writeMSIData(msi_obj, imzml_path, bundle = FALSE)  # kalder writeImzML() under motorhjelmen
#   # Bem.: skriver også .ibd (og evt. .tsv for feature/pixelData)
#   
#   # 2) Upload begge filer til GridFS
#   fs <- gridfs(url = mongo_url, db = db, prefix = prefix)
#   fs$upload(imzml_path, name = paste0(base_name, ".imzML"))
#   fs$upload(file.path(tmp, paste0(base_name, ".ibd")), name = paste0(base_name, ".ibd"))
#   
#   # (Valgfrit) Upload evt. de .tsv-filer Cardinal skrev
#   tsvs <- list.files(tmp, pattern = "\\.tsv$", full.names = TRUE)
#   for (p in tsvs) fs$upload(p, name = paste0(base_name, "_", basename(p)))
# }
# 
# 
# load_msi_from_gridfs <- function(base_name, mongo_url = "mongodb://localhost", db = "cardinal_data", prefix = "fs", memory = FALSE) {
#   tmp <- tempfile("imzml_"); dir.create(tmp)
#   fs  <- gridfs(url = mongo_url, db = db, prefix = prefix)
#   
#   # 1) Hent .imzML og .ibd ned i samme mappe
#   imzml_local <- file.path(tmp, paste0(base_name, ".imzML"))
#   ibd_local   <- file.path(tmp, paste0(base_name, ".ibd"))
#   fs$download(name = paste0(base_name, ".imzML"), path = imzml_local)
#   fs$download(name = paste0(base_name, ".ibd"),   path = ibd_local)
#   
#   # 2) Læs ind igen (hold evt. data fil-backed med memory = FALSE)
#   readImzML(imzml_local, memory = memory)
#   # eller: readMSIData(imzml_local)
# }
# 
# 
# save_msi_to_gridfs(control_mean, "control_mean")
# loaded_mean <- load_msi_from_gridfs(base_name = "control_mean")
# 
# 
# 
# 
