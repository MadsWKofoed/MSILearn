      ...
      div(class = "welcome-container",
        ...
        div(
          ...
          div(
            ...
            div(class = "flow-arrow", HTML("&rarr;")),
            actionButton(ns("obj_pipeline_output"), "Pipeline Output", class = "flow-object-btn")
          ),
...
welcome_module_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$obj_pipeline_output,   active_object("pipeline_output"))
      ...
      if (panel == "processing") {
        return(
          div(
            ...
            p(strong("Main stored objects: "), "Study, Sample, Pipeline, and Pipeline Output."),
            p(strong("Main output: "), "A binned_dataframe Pipeline Output used for clustering and downstream model training.")
          )
        )
      }
      ...
      if (panel == "clustering") {
        return(
          div(
            ...
            p(strong("Main stored objects: "), "Clustering pipeline, clustering Pipeline Output, annotation set, and annotations.")
          )
        )
      }
    ...
    output$object_details <- renderUI({
      ...
      if (obj == "pipeline_output") {
        return(
          div(class = "welcome-detail-box",
            h4("Pipeline Output"),
            p("A Pipeline Output is the output generated by a pipeline."),
            p("For example, a processing pipeline can generate a binned feature matrix stored as a binned_dataframe Pipeline Output."),
