...
process_import_and_summary <- function(imzml_path, ibd_path, imzml_name, run_id,
                                       materialize = c("cache", "process", "lazy")) {
  materialize <- match.arg(materialize)
  
  # --- Sikkerhedstjek ---
  if (!file.exists(imzml_path)) stop("imzML not found: ", imzml_path)
  if (!file.exists(ibd_path))   stop("IBD not found: ", ibd_path)
  
  message("Reading MSI data from original location (no temp copy)...")
  msi_data <- readImzML(imzml_path, memory = FALSE, check = FALSE,
                        mass.range = NULL, resolution = 10, units = "ppm",
                        guess.max = 1000L, as = "auto", parse.only = FALSE,
                        verbose = getCardinalVerbose(), chunkopts = list(),
                        BPPARAM = bpparam())
  
  # --- Materialisér så objektet ikke afhænger af .ibd-sti senere ---
  if (materialize %in% c("cache", "process")) {
    msi_data <- process(msi_data)
  } else {
    warning("Storing a lazy object can break later if the .ibd path is unavailable.")
  }
  msi_data <- ensure_materialized(msi_data)
...
ensure_materialized <- function(msi_obj) {
  # Forsøg at tvinge data ind i hukommelsen
  tryCatch({
    process(msi_obj)
  }, error = function(e) {
    warning("process() failed to materialize object: ", e$message)
    msi_obj
  })
}

verify_ibd_access <- function(msi_obj) {
  tryCatch({
    # Hvis .ibd mangler, fejler dette hurtigt
    tmp <- spectra(msi_obj)[1:1, ]
    TRUE
...
process_reference_creation <- function(run_id, control_mean, ref_mz_values, snr,
  ...
  control_SNR_ref <- control_mean %>% peakPick(SNR = snr)
  
  # VIGTIGT: materialisér inden save, så den ikke peger på .ibd
  control_SNR_ref <- ensure_materialized(control_SNR_ref)
...
process_msi_pipeline <- function(imzml_path, ibd_path, imzml_name,
  ...
  if (raw_exists) {
    ...
    if (!verify_ibd_access(msi_data)) {
      message("RAW artifact depends on missing .ibd. Re-importing from original files...")
      step1 <- process_import_and_summary(
        imzml_path, ibd_path, imzml_name, run_id,
        materialize = "cache"
      )
    ...
    if (nrow(control_mean_artifact) > 0) {
      ...
      control_mean <- summarizeFeatures(msi_data, "mean")
      control_mean <- ensure_materialized(control_mean)
      save_stage_to_mongo(control_mean, run_id, "control_mean", sample_name = imzml_name)
    }
    # Hvis control_mean blev loadet fra Mongo, så materialisér
    if (exists("control_mean") && !verify_ibd_access(control_mean)) {
      message("control_mean depends on missing .ibd. Recomputing from RAW...")
      control_mean <- summarizeFeatures(msi_data, "mean")
      control_mean <- ensure_materialized(control_mean)
      save_stage_to_mongo(control_mean, run_id, "control_mean", sample_name = imzml_name)
    } else if (exists("control_mean")) {
      control_mean <- ensure_materialized(control_mean)
    }
    ...
    step1 <- process_import_and_summary(imzml_path, ibd_path, imzml_name, run_id, materialize = "cache")
    msi_data <- step1$msi_data
    control_mean <- step1$control_mean
  }
  ...
  if (!verify_ibd_access(control_SNR_ref)) {
    message("Existing reference points to missing .ibd. Rebuilding reference...")
...
test <- readImzML("tumorinfiltrat.imzML")
test <- process(test)
spectra(test)[1:5, 1:5]
...
imzml_name <- "tumorinfiltrat.imzML"

run_id <- process_msi_pipeline(
  imzml_path = imzml_file,
  ibd_path = ibd_file,
  imzml_name = imzml_name,
  ref_mz_values = mz_ref$x,
  ref_source = "Input",
  ref_name = "113_features",
  snr = 1.9,
  tolerance = 0.5
)
...
test_dir <- "/tmp/gridfs_test"
dir.create(test_dir, showWarnings = FALSE)

grid$download("tumorinfiltrat__20251107_093307.ibd", test_dir)
